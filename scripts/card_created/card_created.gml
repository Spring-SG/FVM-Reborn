...
function card_created(plant_inst, col, row) {
	...
	if(global.network.mode=="client"&&!global.network.plant_able){
		...
		if (plant_inst.object_index == obj_magic_chicken) {
			...
			_meta = "{\"target_card\":\"" + _target + "\"}";
		}
		send_message(global.network.server_socket, MSG_UNIT_REQUEST, level, col, row, skill, shape, object_get_name(plant_inst.object_index), _meta);
		return;
	}
	...
	if(global.network.mode=="server"){
		...
		for (var i = 0; i < _size; i++) {
			var _socket = _list[i];
			send_message(_socket, MSG_SPAWN_UNIT, global.network.net_instance_count, level, col, row, skill, shape, object_name, _meta);
		}
